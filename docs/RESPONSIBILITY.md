...
Four groups: what the system *says*, who it says it *to*, what it's *built on*, and
...
**6. Default to the smallest thing that clears the bar.** The smallest model, the
fewest calls, the least data retained, the narrowest tool permissions. This is the one
place where the ethical answer and the boring engineering answer are the same answer:
it's cheaper, faster, lower-footprint, less to leak, and less to explain to a regulator.
When you deviate, deviate on purpose and be able to say why.
...
The public argument about training data is mostly about what the frontier labs
scraped, which you don't control. The part you *do* control is every dataset you add:
your fine-tuning set, your RAG corpus, your eval set, your logs. That's a real
responsibility with a small number of concrete questions.
...
code arrives with no licence attached. Neither is exotic; both need a policy rather
than a vibe. If you ship generated code, review it like third-party code, because
...
This is the part of the debate that has moved fastest, and the part most likely to be
missing from an engineering-focused course. The system is fluent, patient, available
at 3am, and it never gets bored of you. That combination does things to people that a
search box doesn't.
...
**Sycophancy is a measurable failure, so measure it.** Agreement feels like quality:
the model that tells the user they're right rates well and retains well, and is worse
...
crisis is not a moderation category you silently refuse and move on from; the
responsible behavior is to respond plainly, surface real human resources, and not
...
model, which hardware, batch size, whether training is amortized in, whether data
centre cooling and embodied carbon are counted. Anyone quoting you one confident
number for "a query" is rounding away most of the question. Treat specific figures the
way [MODELS.md](MODELS.md) treats prices: correct on a date, for a setup, and worth
re-checking.
...
environmental argument, and pretending otherwise is its own kind of dishonesty. It is,
however, the part in your hands.
...
1. **Errors become actions.** A hallucinated fact is a bad sentence; a hallucinated
   tool call is a sent email, a deleted row, a charged card. The eval bar rises because
   the failure mode escaped the chat window.
...
3. **Injection becomes a responsibility issue, not just a security one.** An agent
   reading untrusted text can be steered into acting against the user it's serving.
   That's [SAFETY.md](SAFETY.md)'s territory technically, but the harm lands here.

The practical rule: **sort actions by reversibility, not by difficulty.** Reading is
...
> **Dated: August 2026.** This area is moving quarterly. Treat the specifics below the
> way [MODELS.md](MODELS.md) treats prices: verified on a date, and your job is to
> re-check. This is orientation, not legal advice; if you ship into a regulated
> domain, get a real opinion.

Two things worth knowing even if you never read a statute:
...
2 August 2026**, which is the part that touches ordinary products: telling users
they're interacting with an AI, and marking synthetic media.
...
is the one this page argues for on its own merits: recurring reminders that the user is
talking to a machine (not just once at the start), protocols for recognizing
...
The engineering takeaway is not "learn compliance." It's that **the checklist below
increasingly has legal weight**, and the two cheapest items on it (know which risk tier
you're in, and keep records of your evals) are the ones teams skip.
...
An ethics page that presents every question as settled would break that rule on its own

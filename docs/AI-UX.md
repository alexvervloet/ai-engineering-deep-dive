...
Most AI interfaces are designed around the demo, where the answer is right and
arrives fast. Production is the other cases: the model is unsure, the retrieval
missed, the tool failed, the answer is confidently wrong, or the user asked something
the system should decline. If those states were an afterthought, the interface will
present a wrong answer with exactly the same confident typography as a right one, and
the user has no way to tell them apart.

The practical test for any AI feature: **can the user tell a good answer from a bad
one without already knowing the answer?** If not, you have moved your accuracy
problem onto the user and called it a feature.

---

## 1. Disclosure: say it is AI, once, clearly
...
## 2. Uncertainty: show it where it is actionable

Showing uncertainty is not the same as showing a number. A confidence score of 0.73
means nothing to a user and, for most model setups, is not calibrated anyway. What
...
The strongest uncertainty signal is usually **the absence of a source**. If your
system can say "I found this in these three documents" when it is grounded, then the
grounded and ungrounded cases look different without you having to explain a score.
...
A citation is a promise the user can check. Three requirements, in order of how often
they are broken:
...
that reads like a refusal makes them think they did something wrong. And a silent
degradation, where the system quietly answers worse because a provider is down, is
the one that costs you trust permanently. Say it.

**Write refusals as boundaries, not accusations.** "I cannot give medical advice.
...
Thumbs up and down are cheap to add and mostly useless on their own, because you get
a signal with no referent. Make feedback actionable:
...
than they report subtly wrong output. Confidently wrong answers are the ones users do
not flag, because they believed them.
...
- **Carry the context across.** Making someone re-explain to a human what they just
  ...
  an obstacle.
- **Be honest about wait times.** "A person will reply within a day" beats a queue
...
- **Confidence theatre.** A precise-looking score that is not calibrated. It buys
  trust the system has not earned.
...
- **Silent degradation.** Quietly answering worse during an outage, with an interface
  that looks exactly as confident as usual.
...
If you have shipped an AI feature and none of this:

1. **Design the "I do not know" state.** It is the highest-value hour of UX work in
   any AI product, and most teams have never done it.

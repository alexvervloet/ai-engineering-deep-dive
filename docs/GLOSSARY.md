...
topics, new wording), so a model starts answering things it was never good at. No
...
**Feedback loop**: capturing thumbs up and thumbs down on real answers to build eval and training
data. *(Production)*
...
cannot decide which checks were needed. *(Testing & Delivery §23.1)*
...
its generator, seed, first failure, and shrunk witness. *(Testing & Delivery §23.3)*
...
model behavior, never carry the test's expected answer. *(Testing & Delivery §23.4)*
...
not bit for bit. *(Testing & Delivery §23.5)*
...
zero-based index; conflating them is the classic p95 bug. *(Testing & Delivery §23.5)*
...
cannot prove the lower bound. *(Testing & Delivery §23.9)*
...
user is exposed. *(Testing & Delivery §23.11)*
